...
def linkrescore(s,jmap):
	j = encode_for_url( jmap[s] )
	print j,
	return r'<a href="rescore?json='+j+'">'+s+'</a>'

def showlinks(df,jmap):
	fmtr = ft.partial(linkrescore,jmap=rescore_dat)
	h = df.to_html(formatters=dict(file=fmtr),escape=False)
	HTML(h)

if __name__ == '__main__':
	#df = read_sicdock("/work/sheffler/production/I53/1woz.tcdock")
